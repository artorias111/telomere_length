...
load('../processed_data/cbrqx6711_model.Rdata')

cbraex5 <- brig_geno %>% dplyr::filter(POS>2996606 & POS<2999968) 
...
cbraex5_plot <- brig_temp %>% ggplot(aes(x=POS,y=length,group=POS)) + 
  ...
  scale_x_continuous(lim=c(2996606,2999968))+
...
p <- plot_grid(cbrqx6711_model,cbraex5_plot,ncol=1, align='v',rel_widths=c(1,1),rel_heights = c(1,12))
