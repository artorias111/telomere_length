...
brig_genes <- read.delim("../data/briggsae/goi/brig_goi.tsv")

# load independent tests result
total_independent_tests <- read.table("../data/briggsae/nemascan_runs/Analysis_Results-20220308/Genotype_Matrix/total_independent_tests.txt", quote="\"", comment.char="", stringsAsFactors=FALSE)

independent_test_cutoff <- -log10(0.05/total_independent_tests[[1]])

# load processed mapping data. 
processed_mapping <- read.delim("../data/briggsae/nemascan_runs/Analysis_Results-20220308/Mapping/Processed/processed_length_AGGREGATE_mapping.tsv", stringsAsFactors=FALSE) %>%
